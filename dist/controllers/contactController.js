...
exports.updateContactStatus = exports.getContactRequests = exports.createContact = void 0;
...
exports.createContact = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const preferred_contact = req.body.preferred_contact || req.body.preferredContact;
    ...
    const recentContact = await prisma_1.default.contact_requests.findFirst({
        where: {
    ...
    // บันทึกลงฐานข้อมูล
    const newRequest = await prisma_1.default.contact_requests.create({
        data: {
    ...
    await (0, auditLogger_1.logAudit)(req, 'CREATE_CONTACT_SUCCESS', `Public contact form submitted successfully (Name: ${first_name.trim()} ${last_name.trim()}, Email: ${email.trim().toLowerCase()}, Request ID: ${newRequest.id})`, null);
  ...
  <tr>
    <td style="padding:16px 30px 24px;text-align:center">
      ...
      <p style="color:#cbd5e1;font-size:9px;margin:8px 0 0">แจ้งเตือนอัตโนมัติจากระบบจัดการเว็บไซต์ ปปง.</p>
            ...
            console.log(`[Contact] Email sent to ${supervisors.length} supervisor(s)`);
        }
    }
    catch (emailErr) {
...
exports.getContactRequests = (0, express_async_handler_1.default)(async (req, res) => {
    const requests = await prisma_1.default.contact_requests.findMany({
        orderBy: {
            created_at: 'desc',
        },
    });
...
exports.updateContactStatus = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const oldRequest = await prisma_1.default.contact_requests.findUnique({
        where: { id },
    });
    if (!oldRequest) {
        res.status(404).json({
    ...
    const updatedRequest = await prisma_1.default.contact_requests.update({
        where: { id },
        data: { status },
    });
    await (0, auditLogger_1.logAudit)(req, 'UPDATE_CONTACT_STATUS_SUCCESS', `Admin updated contact request status (Request ID: ${id}, Old status: "${oldRequest.status}", New status: "${status}")`, adminUser?.id);
    res.status(200).json({
        success: true,
